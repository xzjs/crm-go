...
type Task struct {
	...
	Name      string
	Status    int
	StartTime time.Time `orm:"type(datetime);auto_now_add"`
...
// Id doesn't exist
func GetTaskById(id int64) (v *Task, err error) {
	o := orm.NewOrm()
	v = &Task{Id: id}
	if err = o.QueryTable(new(Task)).Filter("Id", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
